"""Implement `falcon-secure init` — update pyproject.toml with stubs path and profiles."""
...
def resolve_stubs_dir() -> Path:
    ...
    """
    import falcon_secure
    pkg_dir = Path(falcon_secure.__file__).parent
...
def _apply_strict_falcon_secure_config(ps_cfg: dict, force: bool) -> bool:
    """Tighten [tool.falcon_secure]. Return True if changed."""
    strict_cfg = {
        "allow_tags": ["general"],
...
def init(
    ...
    3. Add [tool.falcon_secure] skeleton if missing.
    4. If profile='strict', apply strict knobs to mypy/pyright/ruff + tighten falcon_secure.
    ...
    if "falcon_secure" not in toml_data["tool"]:
        if profile == "basic":
            toml_data["tool"]["falcon_secure"] = {
        ...
        changed = True
        print("Added [tool.falcon_secure] skeleton")
    elif profile == "strict":
        if _apply_strict_falcon_secure_config(toml_data["tool"]["falcon_secure"], force):
            changed = True
            print("Tightened [tool.falcon_secure] for strict profile")
...
def _generate_precommit_config(checker: str, profile: str) -> str:
    ...
    yaml_content = "# Generated by falcon-secure init\n"
    yaml_content += "repos:\n"
    ...
    yaml_content += "    hooks:\n"
    yaml_content += "      - id: falcon-secure-audit\n"
    yaml_content += "        name: falcon-secure audit\n"
    yaml_content += "        entry: falcon-secure audit .\n"
