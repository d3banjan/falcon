"""Test 03: shelf.items() triggers Unsafe[Any].

Two assertions:
  - Runtime proof: Items from shelf deserialize payload values.
...
FIXTURE = Path(__file__).parent.parent / "fixtures" / "fix_shelve_03_items.py"


def test_runtime_rce_proof(tmp_path: Path) -> None:
    """Payload executes when iterating over shelf.items()."""
