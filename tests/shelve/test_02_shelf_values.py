"""Test 02: shelf.values() triggers Unsafe[Any].

Two assertions:
  - Runtime proof: Values from shelf deserialize payload.
...
FIXTURE = Path(__file__).parent.parent / "fixtures" / "fix_shelve_02_values.py"


def test_runtime_rce_proof(tmp_path: Path) -> None:
    """Payload executes when iterating over shelf.values()."""
