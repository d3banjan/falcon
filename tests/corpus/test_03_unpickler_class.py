"""Test 03: pickle.Unpickler().load() path.

Two assertions:
  - Runtime proof: subprocess executes payload via Unpickler().load().
...
from tests.corpus.payloads import rce_via_unpickler_class
from tests.conftest import mypy_check

FIXTURE = Path(__file__).parent.parent / "fixtures" / "fix_03_unpickler_class.py"


def test_runtime_rce_proof(tmp_path: Path) -> None:
    """Payload executes via Unpickler().load() in subprocess."""
    marker = tmp_path / "rce_marker.txt"
    payload = rce_via_unpickler_class(str(marker))
